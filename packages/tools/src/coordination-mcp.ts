...
const strings = new Set(['task', 'commandId', 'query', 'name', 'id', 'interpretation']);
const numbers = new Set(['revision', 'deadline', 'area', 'offset']);
const arrays = new Set(['steps', 'evidence', 'refs', 'resultingTasks', 'supersededTasks']);
export function toolCatalog(c: Coordinator, role: string) {
  return Object.entries(operations).filter(([name, op]) => name !== 'submit' && c.agent(role).definition.tools.includes(op.group)).map(([name, op]) => ({
    ...
    inputSchema: { type: 'object', properties: Object.fromEntries(op.keys.map(key => [key,
      key === 'task' && ['propose', 'revise'].includes(name) ? { type: 'object' } :
      strings.has(key) ? { type: 'string' } : numbers.has(key) ? { type: 'integer', minimum: 0 } :
      arrays.has(key) ? { type: 'array', items: ['evidence', 'resultingTasks', 'supersededTasks'].includes(key) ? { type: 'string' } : { type: 'object' } } : { type: 'object' }])), required: op.keys, additionalProperties: false },
  }));
}
...
export class CoordinationMcp {
  ...
  constructor(private c: Coordinator, private gateway: CoordinationGateway,
    private observed: (binding: SessionBinding, name: string, args: unknown, result: unknown) => void = () => {}) {}
  ...
  async call(token: string, name: string, args: unknown) {
    ...
    if (g.binding) this.observed(g.binding, name, args, result);
    return result;
  }
  async listen() {
