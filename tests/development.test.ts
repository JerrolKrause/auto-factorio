...
describe('development process and recovery helpers',()=>{
 ...
 it('routes the ordinary dashboard through shared RCON and reconciles its profile-scoped measurement fence before operator recovery',async()=>{const source=await readFile('scripts/dashboard.ts','utf8'),reconcile=source.indexOf('await reconcileWorkshopMeasurementFence(profile.dir'),operator=source.indexOf('operator = new Operator');expect(source).toContain("new SerialPort(await waitForServer(profile))");expect(source).toContain('fenceDirectory:profile.dir');expect(reconcile).toBeGreaterThan(0);expect(operator).toBeGreaterThan(reconcile);});
});
