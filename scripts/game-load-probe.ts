...
try{
 ...
 await assert.rejects(()=>client.request({op:'submit',batch:{commandId:'readback-new',epoch:o.epoch,session:o.session,task:'phase03-actions',revision:1,actor:'builder-1',surface:'nauvis',grant:{id:'test-area',generation:1},deadline:Number(o.tick)+100,steps:[{kind:'walk',position:{x:0,y:0}}]}}),/restore_requires_phase04_barrier/);
