...
import { readProfile, waitForServer } from './dev/game-processes.js';
...
let operator: Operator; let runtime: DurableRuntime; let closePort = () => {};
if (process.argv.includes('--fixture')) {
  const fixture = await dashboardFixture(directory, true); operator = fixture.operator; runtime = fixture.runtime;
} else {
  const file = value('--profile-file'); if (!file) throw new Error('Use --profile-file for a dedicated project game, or --fixture for synthetic UI demonstration');
  const profile = await readProfile((JSON.parse(await readFile(file, 'utf8')) as { dir: string }).dir);
  const port = await waitForServer(profile); closePort = () => port.close();
  const game = new GameClient(port, () => {}); const life = new Lifecycle(port, game, () => {});
  const control = await life.inspect();
...
const server = dashboard(operator); const origin = await server.listen(Number(value('--port') ?? 0));
const result = { origin, url: origin + '/#cap=' + server.capability, directory, synthetic: process.argv.includes('--fixture') };
