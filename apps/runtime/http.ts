...
export function dashboard(operator: Operator, assets = path.resolve('apps/dashboard/dist'), options:DashboardOptions = {}) {
  ...
  app.post('/api/workshop/launch', async request => {if(!workshopRuntime)throw new Error('Workshop execution host unavailable');const input=record(request.body),raw=record(input.assignment);const supplied=raw.learning&&typeof raw.learning==='object'?record(raw.learning):{};return workshopRuntime.launch({...raw,learning:{cadence:supplied.cadence??'after-session',batchSessions:supplied.batchSessions??5,candidateCap:supplied.candidateCap??3,attemptsPerCandidate:supplied.attemptsPerCandidate??2,autoActivate:supplied.autoActivate??true}});});
